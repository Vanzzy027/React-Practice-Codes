...
// Updated FormValues to reflect possible optional fields for update
type FormValues = {
  movie_name: string
  release_date: string
  is_watched?: boolean // Made optional as it's not in the initial add form
}
...
function App() {
  ...
  const fetchMovies = async () => {
    setLoading(true);
    try {
      const response = await axios.get<MovieToWatch[]>('http://localhost:8080/api/movies');
  ...
  // --- NEW: Update Movie Function ---
  const updateMovie = async (id: number, newName: string) => {
    try {
      const response = await axios.put(`http://localhost:8080/api/movies/${id}`, { movie_name: newName });
  ...
  // --- NEW: Toggle Watched State Function ---
  const toggleWatched = async (movie: MovieToWatch) => {
    try {
      const newIsWatched = !movie.is_watched;
      const payload = {
        movie_name: movie.movie_name,
        release_date: movie.release_date.split('T')[0], // Assuming backend expects 'YYYY-MM-DD'
        is_watched: newIsWatched
      };
      
      const response = await axios.put(`http://localhost:8080/api/movies/${movie.movie_id}`, payload);
      alert(response.data.message);
      fetchMovies(); // Refresh the movie list after toggle
    } catch (error) {
      console.error('Error toggling watched status:', error);
      alert('Failed to toggle watched status.');
    }
  };
  ...
  return (
    <div style={{
      ...
      {/* Add Movie Form (remains the same) */}
      <div style={{
        background: 'white',
        padding: '15px',
        maxWidth: '600px',
        ...
        {loading ? (
          ...
          movies.map(movie => (
            <MovieCard 
              ...
              toggleWatched={toggleWatched} // Pass the new toggleWatched function
              updateMovie={updateMovie} // Pass the new updateMovie function
            />
          ))
        )}
      </div>
